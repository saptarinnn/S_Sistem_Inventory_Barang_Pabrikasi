...
<div class="w-full py-5 mx-auto">
    ...
    <div class="p-4 bg-white shadow rounded-xl sm:p-7">
        ...
        <form method="post" action="<?= site_url('barang/' . $barang->kode . '/update') ?>">
            <?= csrf_field(); ?>
            <!-- Kode Barang -->
            <div class="mb-4">
                <label for="kode" class="block mb-2 font-bold">Kode Barang</label>
                <input type="text" id="kode" maxlength="6" name="kode" class="block w-full px-3 py-2 font-bold border border-gray-200 rounded-lg" placeholder="Input kode barang" required value="<?= $barang->kode ?>">
                <!-- Error -->
            ...
            <div class="mb-4">
                ...
                <select name="satuan_id" id="satuan_id" required class="block w-full px-3 py-2 font-bold border border-gray-200 rounded-lg">
                    ...
                    <?php foreach ($semuaSatuan as $satuan) : ?>
                        <option value="<?= $satuan->id ?>" <?= ($satuan->id == $barang->satuan_id) ? 'selected' : '' ?>><?= ucwords($satuan->nama_satuan) ?></option>
                    <?php endforeach; ?>
                </select>
                <!-- Error -->
            ...
            <div class="mb-4">
                ...
                <select name="pemasok_id" id="pemasok_id" required class="block w-full px-3 py-2 font-bold border border-gray-200 rounded-lg">
                    ...
                    <?php foreach ($semuaPemasok as $pemasok) : ?>
                        <option value="<?= $pemasok->id ?>" <?= ($pemasok->id == $barang->pemasok_id) ? 'selected' : '' ?>><?= ucwords($pemasok->nama) ?></option>
                    <?php endforeach; ?>
                </select>
                <!-- Error -->
            ...
            <div class="mb-4">
                ...
                <input type="text" id="nama_barang" name="nama_barang" class="block w-full px-3 py-2 font-bold border border-gray-200 rounded-lg" placeholder="Input nama barang" required value="<?= $barang->nama_barang ?>">
                <!-- Error -->
            ...
            <div class="mb-4">
                ...
                <input type="text" id="tempat" name="tempat" class="block w-full px-3 py-2 font-bold border border-gray-200 rounded-lg" placeholder="Input tempat" value="<?= $barang->tempat ?>">
                <!-- Error -->
