...
<div class="w-full py-5 mx-auto">
    ...
    <div class="p-4 bg-white shadow rounded-xl sm:p-7">
        <div class="flex justify-end">
            <a href="<?= site_url('barang-masuk/create') ?>" class="px-3 py-1 pb-2 font-semibold text-white transition duration-300 rounded-md bg-slate-500 hover:bg-slate-600">Tambah Data Data</a>
        </div>
                        ...
                        <table class="min-w-full divide-y divide-slate-200" id="dataTableConf">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 font-semibold capitalize text-slate-900 text-start">Jmlh. BrgMasuk</th>
                            ...
                            <tbody class="divide-y divide-slate-200">
                                <?php foreach ($semuaBarangMasuk as $barangMasuk) :  ?>
                                    <tr>
                                        <td class="font-semibold text-slate-900"><?= $barangMasuk->tgl_barangmasuk ?></td>
                                        <td class="font-semibold text-slate-900"><?= strtoupper($barangMasuk->kode_barangmasuk) ?></td>
                                        <td class="font-semibold text-slate-900"><?= ucwords($barangMasuk->nama_barang) ?></td>
                                        <td class="font-semibold text-slate-900"><?= ucwords($barangMasuk->jumlah_barangmasuk) ?></td>
                                        ...
                                        <td class="flex gap-2">
                                            <a onclick="return confirm('Yakin Hapus Data ?')" href="<?= site_url('barang-masuk/' . $barangMasuk->kode_barangmasuk . '/delete') ?>" class="p-2 font-semibold text-red-800 bg-red-200 rounded-lg hover:bg-red-300">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4">
