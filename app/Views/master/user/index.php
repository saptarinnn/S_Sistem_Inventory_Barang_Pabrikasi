                        ...
                        <table class="min-w-full divide-y divide-slate-200" id="dataTableConf">
                            ...
                            <tbody class="divide-y divide-slate-200">
                                <?php foreach ($users as $user) :  ?>
                                    <tr>
                                        <td class="font-semibold text-slate-900"><?= ucwords($user->fullname) ?></td>
                                        <td class="font-semibold text-slate-900"><?= ucwords($user->username) ?></td>
                                        <td class="font-semibold text-slate-900"><?= ucwords($user->role) ?></td>
                                        <td class="flex gap-2">
                                            <?php if ($user->role != 'admin') : ?>
                                                <a href="<?= site_url('users/' . $user->id . '/edit') ?>" class="p-2 font-semibold text-yellow-800 bg-yellow-200 rounded-lg hover:bg-yellow-300">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4">
                                                        <path d="M13.488 2.513a1.75 1.75 0 0 0-2.475 0L6.75 6.774a2.75 2.75 0 0 0-.596.892l-.848 2.047a.75.75 0 0 0 .98.98l2.047-.848a2.75 2.75 0 0 0 .892-.596l4.261-4.262a1.75 1.75 0 0 0 0-2.474Z" />
                                                        <path d="M4.75 3.5c-.69 0-1.25.56-1.25 1.25v6.5c0 .69.56 1.25 1.25 1.25h6.5c.69 0 1.25-.56 1.25-1.25V9A.75.75 0 0 1 14 9v2.25A2.75 2.75 0 0 1 11.25 14h-6.5A2.75 2.75 0 0 1 2 11.25v-6.5A2.75 2.75 0 0 1 4.75 2H7a.75.75 0 0 1 0 1.5H4.75Z" />
                                                    </svg>
                                                </a>
                                                <a onclick="return confirm('Yakin Hapus Data ?')" href="<?= site_url('users/' . $user->id . '/delete') ?>" class="p-2 font-semibold text-red-800 bg-red-200 rounded-lg hover:bg-red-300">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4">
